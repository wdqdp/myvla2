#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
"""Manual phase-prompt ablation for V4, V5, V5.2, and V7 through V7.4."""
...
DATA_PROFILE_SPECS = {
    ...
    "rotation_phase_v7_2_adjustment": (
        PHASE_PROMPT_PROFILE_V2,
        "phase_prompt_h30_terminal_hold_native_reexecution_event_gap_filtered",
    ),
    ...
    "rotation_phase_v7_4_adjustment": (
        PHASE_PROMPT_PROFILE_V2,
        "phase_prompt_h30_v7_3_pre_adjustment_idle_compressed",
    ),
}
...
def rotation_targets(direction: str, magnitude: str) -> tuple[str, str]:
    ...
    if magnitude not in {"moderately", "slightly"}:
        raise ValueError("rotation magnitude must be 'moderately' or 'slightly'")
    return (
        failure_reason_text(direction, "appropriate"),
        recovery_plan_text(direction, magnitude, "none", "moderately"),
    )
...
def deterministic_action_noise(seed: int, *, phase: Phase, index: int) -> np.ndarray:
    ...
    phase_id = {"execution": 0, "reposition": 1, "adjustment": 2}[phase]
...
def _start_forced_recovery(
    *,
    ...
    # Capture only after the old EXECUTION chunk has been stopped. Resetting the
...
def _request_action_chunk(
    *,
    ...
    if args.prompt_profile in {PHASE_PROMPT_PROFILE, PHASE_PROMPT_PROFILE_V2}:
        ...
        # V4 has only the old execution/recovery distinction.  Both manual
        # recovery phases intentionally receive the same old recovery prompt.
    ...
    if args.forced_failure_reason and args.forced_failure_reason in prompt:
    ...
    payload = runtime.build_payload(
        ...
        qpos=observation.qpos,
        # No-history servers reject these keys even when the arrays are empty.
        # build_payload omits both fields when both values are None.
        state_history=(
            observation.state_history if getattr(args, "use_state_history", True) else None
        ),
        state_history_mask=(
            observation.state_history_mask if getattr(args, "use_state_history", True) else None
        ),
        prompt=prompt,
    )
    payload.update(
...
def _wait_for_chunk_continue(
    *,
    ...
    while not operator.is_shutdown() and not runtime.shutdown_event.is_set():
        ...
        if signal_value.kind == "select":
            if signal_value.selected_phase is None:
                raise AssertionError("select signal lacks a phase")
            selected_phase = signal_value.selected_phase
            logger.record(
                {
...
def run_ablation(
    ...
    while published_steps < args.max_publish_step and not operator.is_shutdown():
        ...
        # or inferring another EXECUTION chunk.
        ...
        if boundary_signal is not None:
            if _handle_terminal_signal(
                ...
                return
            if boundary_signal.kind != "trigger" or phase != "execution":
        ...
        if _handle_terminal_signal(
            ...
            return
        if control_signal.kind != "trigger" or phase != "execution":
        ...
        # synchronized observation and use it unchanged for the first reposition request.
        frozen = _start_forced_recovery(
            args=args,
            operator=operator,
            captioner=captioner,
