...
lang_embeddings = "pick the orange"
# lang_embeddings = "pick the carrot"
...
def inference_fn(args, config, policy):
    ...
    while True and not rospy.is_shutdown():
        ...
        # convert bgr ro rgb
        image_arrs = [image_tools.resize_with_pad(cv2.cvtColor(img, cv2.COLOR_BGR2RGB),640,480) for img in image_arrs]
        ...
        payload = {
            "state": proprio,
            "images": {
                "cam_high": image_arrs[0].transpose(2, 0, 1),
                "cam_right_wrist": image_arrs[1].transpose(2, 0, 1),
                "cam_left_wrist": image_arrs[2].transpose(2, 0, 1),
            },
    ...
    with torch.inference_mode():
        while True and not rospy.is_shutdown():
            ...
            while t < max_publish_step and not rospy.is_shutdown() and not shutdown_event.is_set():
                ...
                # print("action_buffer shape:",action_buffer.shape," chunk_size:",chunk_size)
