#!/usr/bin/env python3
"""Build V8.1 Stage-A data from the refreshed black-box dataset."""
...
DEFAULT_REFERENCE_V7_2 = DEFAULT_ROOT / "outputs/rotation_v7_2_adjustment/v7_2_boundary_filter.json"
DEFAULT_REFERENCE_V7_4 = DEFAULT_ROOT / "outputs/rotation_v7_4_adjustment/v7_4_prompt_training_index.json"
DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "outputs/rotation_v8_1_adjustment"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reference-v7-2-boundary-file", type=Path, default=DEFAULT_REFERENCE_V7_2)
    parser.add_argument("--reference-v7-4-index-file", type=Path, default=DEFAULT_REFERENCE_V7_4)
...
def _build(args: argparse.Namespace, output_dir: Path) -> tuple[dict[str, Path], dict[str, Any]]:
    ...
    index_path = output_dir / "v8_1_prompt_training_index.json"
    ...
    rows, index, summary = build_v8_1_adjustment_artifacts(
        dataset_dir=args.dataset_dir,
        v4_index_file=args.v4_index_file,
...
def main() -> int:
    ...
    expected = [
        ...
        for name in (
            ...
            "filter_summary.json",
            "v8_1_prompt_training_index.json",
    ...
    if args.dry_run:
        with tempfile.TemporaryDirectory(prefix="v8_1_adjustment_") as temporary:
            paths, summary = _build(args, Path(temporary))
            del paths
        print("dry_run=true; no V8.1 artifacts were retained")
    else:
        paths, summary = _build(args, output_dir)
        print(f"wrote V8.1 adjustment artifacts to {output_dir}")
