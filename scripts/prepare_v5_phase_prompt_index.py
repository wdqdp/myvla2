#!/usr/bin/env python3
"""Create prompt-only V5 phase sidecars on the immutable V4 dataset/index."""
...
from tactile_vla.vla.v5_phase_data import EXPECTED_ATTEMPT_COUNT
from tactile_vla.vla.v5_phase_data import EXPECTED_EPISODE_COUNT
from tactile_vla.vla.v5_phase_data import build_v5_phase_artifacts


DEFAULT_V4_DIR = Path("/data1/outputs/vla/rotation_v4")
DEFAULT_OUTPUT_DIR = Path("/data1/outputs/vla/rotation_v5")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--overrides-file",
        type=Path,
        default=PROJECT_ROOT / "configs" / "rotation_v5" / "phase_boundary_overrides.json",
    )
...
def main() -> int:
    args = parse_args()
    boundaries, actions, index, summary = build_v5_phase_artifacts(
    ...
    if args.dry_run:
        print("dry_run=true; no V5 phase-label files were written")
        return 0
    ...
    index["phase_boundaries_identity"]["file_sha256"] = sha256_file(output_paths["phase_boundaries"])
    index["action_phase_manifest_identity"]["file_sha256"] = sha256_file(
        output_paths["action_phase_manifest"]
    )
    # Replace the authoring override identity with the persisted, versioned
    # copy that future training/checkpoint validation will read.
    index["source_files"]["phase_boundary_overrides"] = {
        "path": str(output_paths["phase_boundary_overrides"].resolve()),
        "sha256": sha256_file(output_paths["phase_boundary_overrides"]),
    }
    ...
    summary.update(
        {
            ...
            "phase_boundary_overrides_sha256": sha256_file(output_paths["phase_boundary_overrides"]),
        }
    )
    _write_json(output_paths["summary"], summary)
    print(f"wrote V5 prompt-only phase artifacts to {args.output_dir}")
    return 0
