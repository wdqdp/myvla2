#!/usr/bin/env python3
"""Build the V7.3 adaptive stop-filtered, phase-pure H30 training index."""
...
from tactile_vla.vla.v7_3_adjustment_data import artifact_hash_payload
from tactile_vla.vla.v7_3_adjustment_data import build_v7_3_adjustment_artifacts
...
DEFAULT_V7_2_DIR = DEFAULT_ROOT / "outputs/rotation_v7_2_adjustment"
DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "outputs/rotation_v7_3_adjustment"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_V7_2_DIR / "v7_2_boundary_filter.json",
    )
...
def main() -> int:
    args = parse_args()
    rows, index, summary = build_v7_3_adjustment_artifacts(
    ...
    if args.dry_run:
        print("dry_run=true; no V7.3 training artifacts were written")
        return 0

    output_dir = args.output_dir.expanduser().resolve()
    manifest_path = output_dir / "action_phase_manifest.jsonl"
    summary_path = output_dir / "filter_summary.json"
    index_path = output_dir / "v7_3_prompt_training_index.json"
    ...
    validate_v7_3_adjustment_training_index(
        index,
        index_path=index_path,
        dataset_dir=args.dataset_dir,
        revalidate_h30_targets=args.verify_h30_targets,
    )
    print(f"wrote V7.3 adjustment artifacts to {output_dir}")
    return 0
