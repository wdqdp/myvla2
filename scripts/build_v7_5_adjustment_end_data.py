#!/usr/bin/env python3
"""Build the V7.5 audited-stop adjustment-end dataset for the V7.4 backbone."""
...
from tactile_vla.vla.v7_5_adjustment_end_data import artifact_hash_payload
from tactile_vla.vla.v7_5_adjustment_end_data import build_adjustment_end_artifacts
from tactile_vla.vla.v7_5_adjustment_end_data import validate_adjustment_end_artifacts
...
DEFAULT_ACTION_INDEX = DEFAULT_ROOT / "outputs/rotation_v7_4_adjustment/v7_4_prompt_training_index.json"
DEFAULT_BOUNDARIES = DEFAULT_ROOT / "outputs/rotation_v7_2_adjustment/v7_2_boundary_filter.json"
DEFAULT_STAGE_A = DEFAULT_ROOT / "outputs/stage_a_action/pi05_delta_tac_rotation_phase_v7_4_no_history/15000"
DEFAULT_OUTPUT = DEFAULT_ROOT / "outputs/rotation_v7_5_adjustment_end_h100"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--boundary-filter-file", type=Path, default=DEFAULT_BOUNDARIES)
...
def main() -> int:
    ...
    required = (
        args.v4_index_file, args.action_index_file, args.boundary_filter_file,
        args.norm_stats_file, args.caption_summary_file, stage_a_config,
    ...
    manifest, index, summary = build_adjustment_end_artifacts(
        ...
        boundary_filter_file=args.boundary_filter_file,
    ...
    if args.dry_run:
        print("dry_run=true; no V7.5 files were written")
        return 0
    ...
    index["training_data_hash"] = sha256_json({key: value for key, value in index.items() if key != "training_data_hash"})
    validate_adjustment_end_artifacts(index=index, manifest=manifest)
    ...
    _write_json(paths["artifact_hashes"], artifact_hash_payload({name: paths[name] for name in ("manifest", "index", "summary")}))
    print(f"wrote V7.5 adjustment_end artifacts to {args.output_dir}")
    return 0
