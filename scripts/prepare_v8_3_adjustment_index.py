#!/usr/bin/env python3
"""Build V8.3 Stage-A data with time-resampled short small-grasp descents."""
...
from tactile_vla.vla.v8_2_adjustment_data import build_boundary_artifact
...
DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "outputs/rotation_v8_3_adjustment"
...
def _build(args: argparse.Namespace, output_dir: Path) -> tuple[dict[str, Path], dict[str, Any]]:
    ...
    index_path = output_dir / "v8_3_prompt_training_index.json"
    ...
    rows, index, summary = build_v8_3_adjustment_artifacts(
        dataset_dir=args.dataset_dir,
        v4_index_file=args.v4_index_file,
...
def main() -> int:
    ...
    expected = [
        ...
        for name in (
            ...
            "filter_summary.json",
            "v8_3_prompt_training_index.json",
    ...
    if args.dry_run:
        with tempfile.TemporaryDirectory(prefix="v8_3_adjustment_") as temporary:
            _, summary = _build(args, Path(temporary))
        print("dry_run=true; no V8.3 artifacts were retained")
    else:
        paths, summary = _build(args, output_dir)
        print(f"wrote V8.3 adjustment artifacts to {output_dir}")
