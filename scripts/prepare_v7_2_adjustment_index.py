#!/usr/bin/env python3
"""Build the V7.2 training index from the manually audited boundary filter."""
...
from tactile_vla.vla.v7_2_adjustment_data import artifact_hash_payload
from tactile_vla.vla.v7_2_adjustment_data import build_v7_2_adjustment_artifacts
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--boundary-filter-file",
        type=Path,
        default=DEFAULT_OUTPUT_DIR / "v7_2_boundary_filter.json",
    )
...
def main() -> int:
    args = parse_args()
    rows, index, summary = build_v7_2_adjustment_artifacts(
    ...
    if args.dry_run:
        print("dry_run=true; no V7.2 training artifacts were written")
        return 0

    output_dir = args.output_dir.expanduser().resolve()
    manifest_path = output_dir / "action_phase_manifest.jsonl"
    summary_path = output_dir / "filter_summary.json"
    index_path = output_dir / "v7_2_prompt_training_index.json"
    hashes_path = output_dir / "artifact_hashes.json"
    existing = [
        path
        for path in (manifest_path, summary_path, index_path, hashes_path)
        if path.exists()
    ]
        ...
        artifact_hash_payload(
            {
                ...
                "boundary_filter": args.boundary_filter_file,
                "filter_summary": summary_path,
                "training_index": index_path,
            }
        ),
    )
    validate_v7_2_adjustment_training_index(
        index,
        index_path=index_path,
        dataset_dir=args.dataset_dir,
        revalidate_h30_targets=args.verify_h30_targets,
    )
    print(f"wrote V7.2 adjustment artifacts to {output_dir}")
    return 0
